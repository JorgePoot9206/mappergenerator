...
import type { ClassicZone } from "@/types";

interface Props {
  zone: ClassicZone;
  onClose: () => void;
  onUpdate: (zoneId: string, updates: Partial<ClassicZone>) => void;
}

const TYPE_LABELS: Record<string, string> = {
  room: "Room", parking: "Parking", office: "Office",
  region: "Region", road: "Road / Corridor", area: "Area", other: "Other",
};

export function ClassicZonePanel({ zone, onClose, onUpdate }: Props) {
  ...
  useEffect(() => {
    ...
    setIsDirty(false);
  }, [zone.id, zone.name, zone.description, zone.notes]);
    ...
    <aside className="w-72 flex-shrink-0 bg-slate-900 border-l border-slate-800 flex flex-col h-full">
      ...
      <div className="flex items-start justify-between p-4 border-b border-slate-800">
        <div className="flex items-center gap-2 min-w-0">
          <span className="w-4 h-4 rounded-full flex-shrink-0 mt-0.5" style={{ background: zone.color }} />
          <div className="min-w-0">
            <h2 className="font-bold text-white text-sm leading-tight truncate">{zone.name}</h2>
            <p className="text-slate-400 text-xs mt-0.5 uppercase tracking-wider">
              {TYPE_LABELS[zone.type] ?? zone.type}
            </p>
          </div>
        </div>
        <button onClick={onClose} className="text-slate-500 hover:text-slate-200 transition-colors flex-shrink-0 p-1 -mr-1">
      ...
      <div className="px-4 py-3 border-b border-slate-800">
        <p className="text-xs text-slate-500 uppercase tracking-wider mb-1.5">Importance</p>
        <div className="flex gap-1">
          {Array.from({ length: 5 }, (_, i) => (
            <span key={i} className={`text-lg ${i < zone.importance ? "text-yellow-400" : "text-slate-700"}`}>★</span>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div>
          <label className="block text-xs text-slate-400 uppercase tracking-wider mb-1.5">Name</label>
          <input
            type="text"
            value={editName}
        ...
        <div>
          ...
          />
          <p className="text-xs text-slate-600 mt-1">Saved to localStorage</p>
        </div>

        <div>
          <label className="block text-xs text-slate-400 uppercase tracking-wider mb-1.5">Position</label>
          <div className="bg-slate-800/50 rounded-lg px-3 py-2 text-xs text-slate-500 font-mono">
            x: {zone.position.x.toFixed(1)}% · y: {zone.position.y.toFixed(1)}%
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-800">
        <button
          ...
          className={`w-full py-2 px-4 rounded-lg text-sm font-medium transition-all ${
            isDirty ? "bg-indigo-600 hover:bg-indigo-500 text-white" : "bg-slate-800 text-slate-500 cursor-not-allowed"
          }`}
        >
